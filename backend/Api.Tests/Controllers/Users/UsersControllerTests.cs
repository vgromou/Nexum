...
public class UsersControllerTests : IDisposable
{
    ...
    [Fact]
    public async Task UpdateUser_AdminUpdatingOtherUser_ShouldReturn200Ok()
    {
        // Arrange - Create another user in the same organization
        ...
        SetupAuthenticatedUser(_userId, _organizationId); // Admin user
        var request = new UpdateUserRequest
        {
            FirstName = "Updated"
        };
        ...
        // Act
        var result = await _controller.UpdateUser(targetUserId, request, CancellationToken.None);

        // Assert
        var okResult = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        var response = okResult.Value.Should().BeOfType<UserInfo>().Subject;
        response.Id.Should().Be(targetUserId);
        response.FirstName.Should().Be("Updated");
    }

    [Fact]
    public async Task UpdateUser_UserUpdatingSelf_ShouldReturn200Ok()
    {
        // Arrange
        SetupAuthenticatedUser(_userId, _organizationId);
        var request = new UpdateUserRequest
        ...
        // Act
        var result = await _controller.UpdateUser(_userId, request, CancellationToken.None);

        // Assert
        var okResult = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        var response = okResult.Value.Should().BeOfType<UserInfo>().Subject;
        response.FirstName.Should().Be("SelfUpdated");
    }

    [Fact]
    public async Task UpdateUser_NonAdminUpdatingSelf_ShouldReturn200Ok()
    {
        // Arrange - Change current user to non-admin
        var membership = await _context.OrganizationMembers.FirstAsync(m => m.UserId == _userId);
        membership.OrganizationRole = OrganizationRole.User;
        await _context.SaveChangesAsync();
    ...
    [Fact]
    public async Task UpdateUser_AdminChangingOrganizationRole_ShouldSucceed()
    {
        // Arrange - Create another user in the same organization
        var targetUserId = Guid.NewGuid();
        var targetUser = new User
        ...
        var okResult = result.Result.Should().BeOfType<OkObjectResult>().Subject;
        var response = okResult.Value.Should().BeOfType<UserInfo>().Subject;
        response.OrganizationRole.Should().Be(OrganizationRole.Manager);
    }

    [Fact]
    public async Task UpdateUser_AdminPromotingToAdmin_ShouldSucceed()
    {
        // Arrange - Create another user in the same organization
        var targetUserId = Guid.NewGuid();
        var targetUser = new User
    ...
    [Fact]
    public async Task UpdateUser_ShouldPersistToDatabase()
    {
        // Arrange
        var targetUserId = Guid.NewGuid();
        var targetUser = new User
        ...
        // Act
        await _controller.UpdateUser(targetUserId, request, CancellationToken.None);

        // Assert - Verify in database
        var user = await _context.Users.FirstAsync(u => u.Id == targetUserId);
        user.FirstName.Should().Be("Persisted");

        var membership = await _context.OrganizationMembers.FirstAsync(m => m.UserId == targetUserId);
        membership.OrganizationRole.Should().Be(OrganizationRole.Manager);
    }

    #endregion

    #region UpdateUser Authorization Tests

    [Fact]
    public async Task UpdateUser_NonAdminUpdatingOtherUser_ShouldThrowForbiddenException()
    {
        // Arrange - Change current user to non-admin
        var membership = await _context.OrganizationMembers.FirstAsync(m => m.UserId == _userId);
        membership.OrganizationRole = OrganizationRole.User;
        ...
        // Act
        var act = () => _controller.UpdateUser(targetUserId, request, CancellationToken.None);

        // Assert
        await act.Should().ThrowAsync<ForbiddenException>()
            .WithMessage("*update other users' profiles*");
    }

    [Fact]
    public async Task UpdateUser_NonAdminChangingOwnRole_ShouldIgnoreRoleChange()
    {
        // Arrange - Change current user to non-admin
        var membership = await _context.OrganizationMembers.FirstAsync(m => m.UserId == _userId);
        membership.OrganizationRole = OrganizationRole.User;
        ...
        {
            FirstName = "Updated",
            OrganizationRole = OrganizationRole.Admin // Should be ignored
        };
        ...
        var response = okResult.Value.Should().BeOfType<UserInfo>().Subject;
        response.FirstName.Should().Be("Updated");
        response.OrganizationRole.Should().Be(OrganizationRole.User); // Role should NOT be changed
    }

    [Fact]
    public async Task UpdateUser_UpdatingUserOutsideOrganization_ShouldThrowForbiddenException()
    {
        // Arrange - Create a user that's NOT in the current organization
        ...
        _context.Users.Add(outsideUser);
        await _context.SaveChangesAsync();
        // Note: No membership for this user in _organizationId
        ...
        // Act
        var act = () => _controller.UpdateUser(outsideUserId, request, CancellationToken.None);
    ...
    [Fact]
    public async Task UpdateUser_DemotingLastAdmin_ShouldThrowBusinessRuleException()
    {
        // Arrange - Current user is the only admin
        SetupAuthenticatedUser(_userId, _organizationId);
        var request = new UpdateUserRequest
        {
            OrganizationRole = OrganizationRole.User // Trying to demote self
        };

        // Act
        var act = () => _controller.UpdateUser(_userId, request, CancellationToken.None);
    ...
    [Fact]
    public async Task UpdateUser_DemotingAdminWhenMultipleExist_ShouldSucceed()
    {
        ...
        var anotherAdminMembership = new OrganizationMember
        {
            Id = Guid.NewGuid(),
            OrganizationId = _organizationId,
            UserId = anotherAdminId,
            OrganizationRole = OrganizationRole.Admin,
            JoinedAt = DateTime.UtcNow
        };
        _context.OrganizationMembers.Add(anotherAdminMembership);
        await _context.SaveChangesAsync();

        SetupAuthenticatedUser(_userId, _organizationId);
        var request = new UpdateUserRequest
        {
            OrganizationRole = OrganizationRole.User // Demote another admin (not self)
        };

        // Act
        var result = await _controller.UpdateUser(anotherAdminId, request, CancellationToken.None);
    ...
    [Fact]
    public async Task UpdateUser_WithNonExistentUser_ShouldThrowNotFoundException()
    {
        // Arrange
        var nonExistentUserId = Guid.NewGuid();
        SetupAuthenticatedUser(_userId, _organizationId);
    ...
    [Fact]
    public async Task UpdateUser_WithExistingEmail_ShouldThrowConflictException()
    {
    ...
    [Fact]
    public async Task UpdateUser_WithExistingUsername_ShouldThrowConflictException()
    {
    ...
    [Fact]
    public async Task UpdateUser_WithOwnEmail_ShouldSucceed()
    {
        // Arrange - Target user updating with their own email
        var targetUserId = Guid.NewGuid();
        var targetUser = new User
        {
            Id = targetUserId,
        ...
        var request = new UpdateUserRequest
        {
            Email = "target@example.com" // Same email
        };
    ...
    [Fact]
    public async Task UpdateUser_WithoutUserId_ShouldThrowUnauthorizedException()
    {
        // Arrange
        SetupAuthenticatedUser(null, _organizationId);
        var request = new UpdateUserRequest { FirstName = "Test" };
